...
import { getArchiveRecord, hardDeleteArchivedRecord } from '@/lib/archive/trash';
import {
...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json().catch(() => null)) as HardDeletePayload | null;
    ...
    await hardDeleteArchivedRecord({
      idToken: verified.idToken,
    ...
    await createActivityLog({
      ...
      idToken: verified.idToken,
      log: {
        action: 'permanently_deleted_recovery_record',
        module: 'recovery',
        target: {
          type: module,
          id,
        ...
        description: `${verified.session.name || verified.session.email} permanently deleted ${recordName} from ${moduleLabel} recovery.`,
        metadata: {
          archivePath: paths.archivePath,
    ...
    console.error(error);
    return Response.json({ error: 'Failed to permanently delete archived record.' }, { status: 500 });
  }
}
